# Final Project ----
# Stat 301-2
# Define, fit, and tune random forest model (complex recipe)
...
rf_params <- extract_parameter_set_dials(rf_wflow_complex)|>
  update(
    mtry = mtry(c(1,33)),
...
# fit workflow/model ----
## Tuning resample
rf_tune_complex <- rf_wflow_complex|>
  tune_grid(
    resamples = hhs_folds,
    grid = rf_grid,
    control = keep_wflow,
...
save(rf_tune_complex,
     file = here("workflow_fitting/complex/rf_tune_complex.rda"))

save(rf_wflow_complex, 
     file = here("workflow_fitting/complex/rf_wflow_complex.rda"))
